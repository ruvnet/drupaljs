/**
 * Local, minimal contracts the `views` module depends on from other Drupal
 * subsystems that are not yet ported (or whose `@drupaljs/*` package is empty at
 * time of writing). Each is a faithful but reduced surface of the real Drupal
 * interface; replace with the shared `@drupaljs/*` type when it lands.
 */
 ...
 *
 * TODO(@drupaljs/access): replace with the shared AccessResult port once the
 * (currently empty) `@drupaljs/access` package lands.
 */
export type AccessVerdict = 'allowed' | 'forbidden' | 'neutral';

/**
 * A minimal, immutable access result with the combinator semantics the views
 * access plugin relies on. Ports the subset of `Drupal\Core\Access\AccessResult`
 * actually used here.
 */
